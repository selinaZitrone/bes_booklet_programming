...
structures and paths, and different operating systems. Therefore, to ensure reproducibility, you need to provide more than the code and the data. Document the exact versions of all packages, libraries, software, and potentially your operating system and hardware.

Below are some basic tips to ensure others can run your code and obtain the same results.

:::{.callout-tip}
**Test Reproducibility**: If you are unsure whether your project is reproducible, send it to a colleague or test it on a different machine.
:::

### Write portable

Portability is crucial for collaborative work and when running code on different servers. One major way to improve portability of your code is to avoid absolute paths and use relative paths instead, ensuring that the script is run from the project root folder (see project organization section).
...
If you e.g. wrote your code on a recent version of R and give it to someone who has not upgraded recently, they may not be able to run your code. Code written for one version of a package may produce very different results with a more recent version. 

Dependency management can be done in a lot of ways, some are basic (but still
useful!) while others are more complex and robust.
In the following you find three levels of complexity to document the dependencies for your projects.
...
In **R**,  use `devtools::session_info()` to get a nicely printed table of all
dependencies. Add this information in your project (e.g. in a readme file).
...
Create a local library with the correct versions of packages used in the project.
This way, the users don't have to use their globally installed software that might
have a different version, but they can use a local project library.

Using a package manager or a virtual environment helps ensure that the correct versions of packages are installed.

:::{.callout-tip}

## Create project local libraries

In **R**, you can use the `renv` package [^2] to manage dependencies. Initialize `renv` in your project:
...
If you are using R, you can have a look at the `rocker` project [^3] that  and project that helps you provide docker containers for R environments [@nust2020, @boettiger2017].

### Namespace conflicts

When using multiple packages, namespace conflicts can occur, where different packages have functions with the same name. This can lead to unexpected behavior in your code. Therefore, it is good practice to prefix functions with the package name to avoid such conflicts and make the dependency explicit.
...
- **Set a Seed**: Explicitly setting a random seed ensures reproducibility. 
- This is important when your code involves random number generation, such as in simulations or machine learning. For this you can use `set.seed(123)` in R, `random.seed(123)` in Python and `Random.seed!(123)` in Julia.
